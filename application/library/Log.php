...
class Log
{
    ...
    public static function init($config = [])
    {
        ...
        if (class_exists( $class )) {
            ...
            die('Class no exists'  .$class);
            // throw new ClassNotFoundException('class not exists:' . $class, $class);
        }

        // 记录初始化信息
        Yaf_Registry::get('config')->application->debug && Log::record('[ LOG ] INIT ' . $type, 'info');
    }
    ...
    public static function save()
    {
        ...
        // TODO 把init的参数放到配置项中
        is_null(self::$driver) && self::init( ['type'  =>  'File'] );
        ...
        if (empty(self::$config['level'])) {
            // 获取全部日志
            $log = self::$log;
            if ( !Yaf_Registry::get('config')->application->debug && isset($log['debug']) ) {
                unset($log['debug']);
            }
        } else {
            // 记录允许级别
